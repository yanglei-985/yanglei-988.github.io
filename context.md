...
<这里粘贴你完整的 <section ...> ... </section> HTML，压缩成单行，不要有换行或缩进>
```
~~~
...
**`<style>` 那行只能中和视觉样式（灰底、等宽字体），无法阻止 `<pre>` 把标签间的缩进/换行渲染成可见字符。** 如果 HTML 保留了换行和缩进，`<pre>` 会把那些空白直接输出，导致行首出现裸数字（1、2、3…）、列表散点、卡片层级垮掉。

**正确做法：打包前先压缩 HTML，去掉所有标签之间的空白，使其成为单行。** 用 Python 一行搞定：
...
压缩后整个 `Gmeek-html` 代码块的结构应该是：
- 第 1 行：` ``` `
- 第 2 行：`Gmeek-html<style>...</style><section...>所有HTML压成一行</section>`
- 第 3 行：` ``` `

共 3 行（不含首尾空行），HTML 内容本身不换行。
...
- **HTML 没压缩就直接包进 Gmeek-html 块 → 线上出现行首数字、列表散点、卡片垮掉**。`<style>` 只管视觉，`<pre>` 的空白渲染问题必须靠压缩 HTML 解决。**每次打包前必须先压缩。**
...
把"包好的整段"做这三项检查:
1. **HTML 是否已压缩**:Gmeek-html 代码块第 2 行（内容行）应该是一整行，不含换行。
2. `<section>` 开标签数 == `</section>` 闭标签数(必须相等,差值=0)。
...
- **S31(标题「AI的黄金内容」= Issue #52,线上 `post/AI-de-huang-jin-nei-rong.html`)**:已用压缩+平衡版 `drafts/s31_min.gmeek.md`(开/闭各 38,HTML 单行,已校验)替换 Issue #52 正文。
- 已正常发布、排版正确的:**Issue #51「AI是最好的杠杆」**(= 会议整理,29 section,渲染无误)。

## 9. 可直接发给下一个 AI 的提示词

> 我在用 Gmeek(GitHub Issues → GitHub Pages)写博客。我要把"公众号风格的 `<section>` + 行内 style HTML"发布上去并保持排版。已知:Gmeek 用 GitHub GFM API 渲染,会清洗裸 HTML,必须把 HTML 包进以 `Gmeek-html` 开头的 fenced 代码块(并加一条中和 `#postBody pre.notranslate` 的 `<style>`)才能还原成真实 HTML;内容必须写进 Issue 正文且至少加 1 个 Label;Issue 的 Preview 标签不代表最终效果。**特别注意：HTML 必须先压缩成单行（去掉标签间所有换行和缩进），否则 `<pre>` 外壳会把空白字符直接渲染出来导致排版垮掉。** 请:① 检查我给你的 HTML 的 `<section>` 开/闭标签是否相等(不等就补全 `</section>`);② 用 `re.sub(r'>\s+<', '><', html)` 压缩 HTML 为单行;③ 按上面的套法把它包好;④ 输出我可直接粘进 Issue 的整段内容。
